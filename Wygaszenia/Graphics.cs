using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;
using System;
using System.Windows.Forms;

namespace Wygaszenia
{
    public class GraphicsHelper
    {
        public Color white = new Color(200, 200, 200);
        public Color gray = new Color(30, 30, 30, 5);
        public float zoomX, zoomY, zoom;
        Panel panelViewport;
        SpriteBatch spriteBatch;
        Texture2D texture;

        public void SetValues(Panel panelViewport_, SpriteBatch spriteBatch_, Texture2D texture_, float zoomX_,  float zoomY_ )
        {
            panelViewport = panelViewport_;
            spriteBatch = spriteBatch_;
            texture = texture_;
            zoomX=zoomX_;
            zoomY= zoomY_;
            if (zoomX > zoomY) zoom = zoomX;
            else
                zoom = zoomY;
            
        }
        public void Point(Vector2 p, Color col)
        {
            spriteBatch.Draw(texture, p, null, col, 0,  new Vector2(0, 0), zoom, SpriteEffects.None, 0);

        }



        public void Line(GraphicsDeviceService serv, BasicEffect efect, Vector3 vector1, Vector3 vector2, Color color)
        {
            VertexPositionColor[] vertices = new VertexPositionColor[2];
            vertices[0].Position = new Vector3(vector1.X, vector1.Y, vector1.Z);
            vertices[0].Color = color;
            vertices[1].Position = new Vector3(vector2.X, vector2.Y, vector2.Z);
            vertices[1].Color = color;

            //Draw the line
            efect.CurrentTechnique.Passes[0].Apply();
            serv.GraphicsDevice.DrawUserPrimitives<VertexPositionColor>(PrimitiveType.LineList, vertices, 0, 1);

        }


        public void FiledRectangle(GraphicsDeviceService serv, BasicEffect efect, Vector3 v1, Vector3 v2, Vector3 v3, Color color)
        {
            VertexPositionColor[] vertices_tringle = new VertexPositionColor[3];
            vertices_tringle[0].Position = v1;
            vertices_tringle[0].Color = color;
            vertices_tringle[1].Position = v2;
            vertices_tringle[1].Color = color;
            vertices_tringle[2].Position = v3;
            vertices_tringle[2].Color = color;
            efect.CurrentTechnique.Passes[0].Apply();
            serv.GraphicsDevice.DrawUserPrimitives<VertexPositionColor>(PrimitiveType.TriangleList, vertices_tringle, 0, 1);

            vertices_tringle[0].Position = v3;
            vertices_tringle[0].Color = color;
            vertices_tringle[1].Position = v2;
            vertices_tringle[1].Color = color;
            vertices_tringle[2].Position = v1;
            vertices_tringle[2].Color = color;
            efect.CurrentTechnique.Passes[0].Apply();
            serv.GraphicsDevice.DrawUserPrimitives<VertexPositionColor>(PrimitiveType.TriangleList, vertices_tringle, 0, 1);

        }
    }
}
